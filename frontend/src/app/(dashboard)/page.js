...
export default function Home() {
  ...
  const handleSendMessage = async (text) => {
    ...
    if (!user) {
      // Guest flow: Create a temporary guest chat state
        ...
        <div className="relative w-full group max-w-[720px] mx-auto">
          ...
          <div className="relative bg-bg-surface/92 backdrop-blur-xl border border-border p-5 rounded-[22px] shadow-xl">
            <textarea 
              ...
              className="w-full bg-transparent border-none focus:ring-0 text-text-primary resize-none h-32 text-base placeholder:text-text-tertiary"
              onKeyDown={(e) => {
              ...
              <div className="flex items-center gap-base">
                ...
                {user ? (
                  <button 
                    ...
                    className="flex items-center gap-compact text-[10px] font-bold text-accent-red hover:text-accent-red/80 transition-fast uppercase tracking-widest"
                  >
                    <LogOut size={14} />
                    Sign Out
                  </button>
                  ...
                  <button 
                    onClick={() => router.push('/login')}
                    className="flex items-center gap-compact text-[10px] font-bold text-accent-blue hover:text-accent-blue/80 transition-fast uppercase tracking-widest"
                  >
                    <LogIn size={14} />
                    Sign In
                  </button>
